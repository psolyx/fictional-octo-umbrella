...
def run_signoff_bundle(*, repo_root: str, out_evid_root: str, base_url: str | None = None, dry_run: bool = False, out=None):
    ...
    with (bundle_dir / "index.html").open("w", encoding="utf-8", newline="\n") as handle:
        artifact_links = [(path, path) for path in _INDEX_ARTIFACT_ORDER]
        notes = [
            "./scripts/phase5_2_signoff_verify.sh EVID_DIR=./evidence/<bundle-path>",
            "./scripts/phase5_2_signoff_compare.sh A_EVID_DIR=./evidence/<bundle-a> B_EVID_DIR=./evidence/<bundle-b>",
        ]
        handle.write(
            render_signoff_index(
                manifest=manifest,
                artifacts=artifact_links,
                result="PASS" if bool(manifest.get("success", False)) else "FAIL",
                notes=notes,
            )
            + "\n"
        )
