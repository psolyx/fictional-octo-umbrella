...
def test_dm_send_uses_deterministic_msg_id():
    ...
    args = SimpleNamespace(conv_id="c_1", state_dir="state", plaintext="hello", base_url=None)

    with mock.patch("cli_app.mls_poc._load_session", return_value=("https://gw", "st")):
...
def test_dm_tail_routes_and_acks(monkeypatch):
    ...
    monkeypatch.setattr("cli_app.mls_poc._load_session", lambda _base: ("https://gw", "st"))
    ...
    update_calls = []
    monkeypatch.setattr("cli_app.mls_poc.gateway_store.get_next_seq", lambda _conv: 5)
    monkeypatch.setattr(
        "cli_app.mls_poc.gateway_store.update_next_seq",
        lambda _conv, seq: update_calls.append(seq),
    )

    args = SimpleNamespace(conv_id="c_3", state_dir="state", from_seq=None, ack=True, base_url=None)
